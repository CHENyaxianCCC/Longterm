...
plot_lm_results_decrease <- lm_results_decrease %>%
  ggplot(aes(x = first_b_tm, y = first_b)) +
  geom_point(aes(size = n), alpha = 0.75, col = "gray") +
  labs(x = expression(Slope~of~air~temperature~(degree~C~year^-1)),
       y = expression(Slope~of~soil~respiration~(g~C~m^-2~year^-1))) +
  ...
  geom_smooth(mapping = aes(x = first_b_tm, y = first_b),
              ...
              se = T, fill = "skyblue",
              show.legend = FALSE) +
  guides(size = guide_legend("Year (n)")) -> plot_lm_results_decrease

## add average dot
lm_results_decrease_mean <-
  tibble(tm_slope_mean = lm_results_decrease$first_b_tm %>% mean(),
         Rs_slope_mean = lm_results_decrease$first_b %>% mean(),
         Rs_slope_se_mean = lm_results_decrease$first_b_se %>% mean(),
         Year_mean = lm_results_decrease$n %>% mean())

plot_lm_results_decrease +
  geom_point(aes(x = tm_slope_mean, y = Rs_slope_mean),
             col = "black", size = 3.5, data = lm_results_decrease_mean) +
  geom_segment(aes(x = tm_slope_mean, y = Rs_slope_mean - 2 * Rs_slope_se_mean,
                    xend = tm_slope_mean, yend = Rs_slope_mean + 2 * Rs_slope_se_mean),
              col = "black", size = 1,
              data = lm_results_decrease_mean) -> plot_lm_results_decrease  

# 添加 X 轴直方图
subx1 <- ggplot(lm_results_decrease, aes(x = first_b_tm)) +
  geom_histogram(bins = 50, fill = "white", col = "black") +
  theme_void()

# 添加 Y 轴直方图（旋转 90 度）
suby1 <- ggplot(lm_results_decrease, aes(x = first_b)) +
  geom_histogram(bins = 30, fill = "white", col = "black") +
  theme_void() +
  coord_flip()


final_plot_1 <- (subx1 + plot_spacer() + plot_lm_results_decrease + suby1) +
  plot_layout(
    ncol = 2,
    nrow = 2,
    widths = c(4, 1),
